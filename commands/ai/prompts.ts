...
export function buildSystemPrompt(userPrompt: string, context: string): string {
  ...
  const createRules = [
    ...
    `Set input.media_type to a single lowercase label (${knownMediaTypesText()}) inferred from URL and intent; prefer existing media types from context when they fit.`,
    ...
    'Published bookmark data is often sparse. Do not rely on the custom media_type tag (`m`) as a required server-side filter; many good bookmarks omit it even when they are clearly something to read, watch, listen to, or use as an app.',
