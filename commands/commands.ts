...
//
// Implement each subcommand (list, show, delete, drafts, accept, revise,
// decline, ai) for your plugin. New bookmarks are created via AI drafts only.
// ---------------------------------------------------------------------------
...
export async function handleBm({
  ...
  if (sub === 'next') {
    const parsed = parseBmNextCliArgs(rest);
  ...
  if (sub === 'add') {
    return [
      `Bookmarks are added only through !${alias} ai (drafts with full fields—category, tags, media type, queue).`,
      '',
      `Use: !${alias} ai <prompt>`,
    ].join('\n');
  }
  ...
  if (sub === 'types' || sub === 'media-types') {
    if (rest.length > 0) {
      return `Usage: !${alias} types`;
    }
